...
| 1.4 | **Poner `OPEN_REGISTRATION` fuera de producción** | Salió de la revisión de seguridad (8/8). Con esa variable en `1`, cualquiera se registra eligiendo su `unitId` y sin vehículo — y `cobradorDeSuCombi` cae a `unitId` como "su combi", así que un registro con el nombre justo administra a los cobradores de **otra cooperativa**. Es la única puerta cruzada que quedó en pie, y sólo existe si alguien enciende la variable. No se tocó el código porque el banner ya dice que es para demos: lo que hace falta es asegurarse de que el deploy real no la tenga | verificar el deploy |
...
| 4.6 | **El acumulado por unidad no tiene corte por fecha** | Medido (8/8, `COSTOS.md` §3). `/admin/metrics` es la única lectura del sistema que agrupa "todo lo retenido" en vez de un rango, y adentro lleva una subconsulta correlacionada: el costo crece con el **cuadrado** de la flota. A 2000 unidades eran 10 368 ms; con tres índices nuevos quedó en 88. A **20 000 serían 16,6 minutos** sin ellos y ~3,7 s con ellos — y eso es tiempo con la ingesta de GPS parada, porque SQLite comparte hilo. Los índices ya no compran más: la salida es **acotarlo a un rango**, como el resto del panel. No urge a 2000, sí antes de crecer |
