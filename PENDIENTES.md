...
| 1.3 | **Medir un turno de 8 horas** | Toda la app nativa existe por una promesa —el GPS aguanta con la pantalla bloqueada— que solo está comprobada por *varios minutos*. Si a las 3 horas Android la mata, el producto no es lo que decimos que es. No se arregla programando: se mide. **Primer intento (7/9): no llegó a 5 minutos**, con la batería sin restricción. No era Android matando nada: el corte de 15 s del envío era un `setTimeout`, y con la pantalla apagada React Native no corre los timers — un `POST /gps` colgado tapaba todos los siguientes (ráfagas y silencios de minutos en el servidor, «volvió tras 0 s» al prender la pantalla). Corregido: el corte lo hace la propia tarea del GPS (`app/envio.js`, suite `envio`). Hay que volver a medir con el APK nuevo, mirando «enviando hace N s» en la pantalla | 1 turno |
