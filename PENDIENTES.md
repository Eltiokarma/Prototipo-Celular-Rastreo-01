...
| 4.18 | **Los trucos del chofer contra el control: un análisis a fondo** | Pedido el 8/9. El sistema mide al chofer con el teléfono del chofer, y el chofer tiene el teléfono en la mano: hay que enumerar, probar y decidir qué se hace con cada truco para **evitar ser controlado o romper el sistema**. La lista para empezar: **cambiar la hora del teléfono** (hoy el servidor estima el reloj atrasado y lo descuenta —`server/reloj.js`—, un salto hacia atrás deja las posiciones «ya vistas» lo que saltó, y una hora adelantada más de 2 min se descarta; ¿qué pasa con las vueltas, los tramos y las paradas fechadas con esa hora?); **GPS falso** (apps de «fake GPS», modo desarrollador: posiciones perfectas sobre el trazado sin salir del terminal — ¿se distingue por velocidad constante, precisión imposible, sin ruido, saltos?); **modo avión o datos apagados a voluntad** en los tramos que no quiere que se vean (el olvido y la reanudación lo tratan como corte de señal; ¿cuánto silencio es sospechoso?); **matar la app o la optimización de batería «sin querer»**; **marcar «ausente» o «fuera» para que no cuenten la parada o el desvío** (hoy fuera de la cadena no se juzga nada: es exactamente el hueco); **el botón de tráfico para justificar una parada**; **dos teléfonos o dos cuentas** (L9: los dos aceptados por HTTP); **el celular prestado a otra combi**; **cerrar sesión y volver a entrar** para borrar la confirmación y la marca de entrada tardía; **el reloj del servidor**, que es la única fuente de verdad y no se cuestiona. Para cada uno: cómo se ve en los datos, si se puede detectar, si se avisa a Despacho o sólo se anota, y qué NO se va a impedir porque el costo (falsas acusaciones a choferes honestos) es mayor que el truco. Va después de la tanda 4 | ? |
| 4.17 | **El logo grande en la presentación, en vez del ícono** | Opción anotada, no decidida. Hoy la presentación al abrir (`Presentacion` en `app/App.js`) muestra el ícono de la app (112 px) sobre el fondo; la alternativa es el logotipo de Micros Tempo grande, el de la landing. Hace falta el logotipo como imagen en `app/assets/` (PNG con fondo transparente, ancho ~600 px) y cambiar el `source` y el tamaño en `Presentacion`; la pantalla nativa de arranque (`expo-splash-screen` en `app.json`) puede llevar la misma imagen para que el pase no se note. Es un APK nuevo | ? |
| 4.16 | ~~Avisar a los otros choferes que hay tráfico~~ | **HECHO** (8/9), como se conversó: N minutos, automático y de un toque. El servidor marca `parado` a la unidad confirmada en ruta que en 3 min (`PARADA_MS`) avanzó menos de 150 m por el trazado —metros sobre la ruta, no velocidad: la fila que arrastra a paso de hombre cuenta igual—, salvo en los extremos del tramo (el terminal), fuera de ruta o fuera de la cadena. El chofer confirma o avisa antes con un botón de un toque (`trafico`, por WS o `POST /trafico`); la app vibra una vez cuando el servidor lo ve parado y pregunta. Las dos marcas se apagan solas al andar 100 m en un minuto. Al de atrás le llega en la brecha (`aheadEnTrafico`…) y su instrucción pasa a «mantené», nunca «apurá» hacia el embotellamiento; la notificación con la pantalla apagada lo dice igual. Despacho lo ve en la fila de la unidad. Cada episodio queda en `paradas` (dónde, punto del circuito, duración, confirmado, cómo terminó) y `GET /admin/paradas` lo lista: la materia prima para predecir dónde se traba cada ruta. Suites `parada` (pura), `trafico` (servidor) y `hud`. Falta el APK y verlo en la calle; la web del chofer no tiene el botón (la app nativa es la que se reparte) | ✔ |
