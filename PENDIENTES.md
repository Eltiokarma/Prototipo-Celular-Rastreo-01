...
| 1.3 | ~~**Medir un turno de 8 horas**~~ | Toda la app nativa existe por una promesa —el GPS aguanta con la pantalla bloqueada— que solo está comprobada por *varios minutos*. Si a las 3 horas Android la mata, el producto no es lo que decimos que es. No se arregla programando: se mide. **Primer intento (7/9): no llegó a 5 minutos**, con la batería sin restricción. No era Android matando nada: el corte de 15 s del envío era un `setTimeout`, y con la pantalla apagada React Native no corre los timers — un `POST /gps` colgado tapaba todos los siguientes (ráfagas y silencios de minutos en el servidor, «volvió tras 0 s» al prender la pantalla). Corregido: el corte lo hace la propia tarea del GPS (`app/envio.js`, suite `envio`). **Segundo intento (7/9), con ese APK: 4,5 minutos frescos y después el servidor siguió oyendo al teléfono más de 8 minutos, pero con posiciones viejas** — la red del teléfono va en ventanas (Doze o el fabricante, pese al menú de batería) y la cola mandaba las 150 más viejas primero. Tres cosas más: la cola es una sola tanda con la posición de ahora adentro; el servidor descarta como «ya vista» lo repetido o más viejo que lo que tiene (y lo deja en el log con las dos edades, igual que «sin señal»); y la app le PREGUNTA a Android si la optimización de batería sigue activa y lo dice en rojo con el atajo a ajustes (`expo-battery`, APK nuevo). Despacho ve si al teléfono se lo sigue oyendo debajo de «sin señal». **Tercer intento (7/9), con el log nuevo del servidor: la causa de fondo, por fin.** Cada `POST /gps` llegaba en 1 s, el servidor contestaba, y la app no se enteraba de NINGUNA respuesta: reencolaba y reenviaba a los 20 s («150 posiciones, 150 ya vistas») durante 1 h 47 min, hasta prender la pantalla. `fetch` en React Native resuelve con `setTimeout(fn, 0)`, y en bridgeless ni ese corre con la actividad pausada. El pedido pasó a `XMLHttpRequest` con timeout nativo (`app/pedido.js`, suite `pedido`), y se corrigió la cola, que al reencolar lo cortado tiraba las posiciones nuevas («la más nueva de hace 4302 s»). **Cuarto intento (7/9), con ese APK: 13 horas seguidas con la pantalla apagada, sin un solo corte, 4 vueltas contadas.** El turno de 8 h está medido. Quedó un residuo, arreglado después: la tarea recibía las mismas posiciones varias veces (expo-task-manager relanza el job pendiente con todo lo acumulado) y las mandaba —el servidor las descartaba como «ya vistas»—; ahora se filtran en la puerta (`filtrarEntregadas`). Lo que falta: la batería en ese turno, y probarlo en teléfonos Xiaomi/Huawei/Oppo | ✔ (falta batería) |
...
| 4.18 | ~~**Los trucos del chofer contra el control: un análisis a fondo**~~ | **ANÁLISIS HECHO** (10/9): `TRUCOS-2026-09-10.md`, doce trucos, cada uno con cómo se ve en los datos, qué hace el sistema hoy (con archivo), si se detecta, qué hacer y qué NO se va a hacer. Principio: detectar no es acusar; todo se anota, poco se avisa, nada se castiga solo. **Primer paso hecho**: el GPS simulado (T2) — Android marca la posición que sale de una app de «fake GPS», la app la manda como `simulado`, la unidad lleva `gpsSimulado`, Despacho lo ve y la auditoría lo anota (APK, `versionCode` 4). **Paso 1 hecho** (10/9, servidor, sin APK): las tablas `huecos`, `presencia_log` y `anomalias`, los CSV `senal` y `anomalias`, el bloque `senal` por unidad en el resumen del gerente y la sección «Señal y presencia» en Números de Despacho (suite `huecos`). **Pasos 2 y 3 hechos** (10/9): la app manda `precision` y el servidor no juzga desvío ni parada con más de 100 m de error (marca «GPS IMPRECISO», anota y cuenta; APK `versionCode` 5); la heurística de simulación sin flag (`server/farsa.js`: velocidad que no varía, clavado en el trazado, velocidad que no cuadra → `gpsSospechoso` con motivo, «GPS SOSPECHOSO» en Despacho, nunca bloquea); y el contador de avisos de tráfico sin parada medida (`paradas.medida`, columna Tráfico en «Señal y presencia»). Suites `farsa` (pura) y `trucos` (servidor). **Lo que queda es verlo en la calle** con el APK 5 | ✔ |
| 4.17 | **El logo grande en la presentación, en vez del ícono** | Opción anotada, no decidida. Hoy la presentación al abrir (`Presentacion` en `app/App.js`) muestra el ícono de la app (112 px) sobre el fondo; la alternativa es el logotipo de Micros Tempo grande, el de la landing. Hace falta el logotipo como imagen en `app/assets/` (PNG con fondo transparente, ancho ~600 px) y cambiar el `source` y el tamaño en `Presentacion`; la pantalla nativa de arranque (`expo-splash-screen` en `app.json`) puede llevar la misma imagen para que el pase no se note. Es un APK nuevo. **Decidido el 10/9**: se queda la presentación con el nombre y «Control de ruta»; la pantalla nativa pasa a ser sólo el fondo (sin ícono), porque con el ícono encima se veían dos pantallas seguidas. El logo grande queda como opción anotada, no decidida | ✔ pantalla única |
| 4.16 | ~~Avisar a los otros choferes que hay tráfico~~ | **HECHO** (8/9), como se conversó: N minutos, automático y de un toque. El servidor marca `parado` a la unidad confirmada en ruta que en 3 min (`PARADA_MS`) avanzó menos de 150 m por el trazado —metros sobre la ruta, no velocidad: la fila que arrastra a paso de hombre cuenta igual—, salvo en los extremos del tramo (el terminal), fuera de ruta o fuera de la cadena. El chofer confirma o avisa antes con un botón de un toque (`trafico`, por WS o `POST /trafico`); la app vibra una vez cuando el servidor lo ve parado y pregunta. Las dos marcas se apagan solas al andar 100 m en un minuto. Al de atrás le llega en la brecha (`aheadEnTrafico`…) y su instrucción pasa a «mantené», nunca «apurá» hacia el embotellamiento; la notificación con la pantalla apagada lo dice igual. Despacho lo ve en la fila de la unidad. Cada episodio queda en `paradas` (dónde, punto del circuito, duración, confirmado, cómo terminó) y `GET /admin/paradas` lo lista: la materia prima para predecir dónde se traba cada ruta. Suites `parada` (pura), `trafico` (servidor) y `hud`. Falta el APK y verlo en la calle; la web del chofer no tiene el botón (la app nativa es la que se reparte). **Corregido el 10/9**: la parada automática **no se detectaba nunca en la calle** — la ventana de `server/parada.js` descartaba toda muestra más vieja que 3 min y exigía que la primera guardada tuviera 3 min, cosa que con posiciones cada ~10 s (nunca múltiplos exactos) no pasaba jamás; las suites usaban pasos de 10 s exactos y por eso no lo veían. Ahora se guarda la última muestra de antes del borde y la ventana mide de verdad 3 min (suite `parada`, sección «no caen justo en el borde») | ✔ |
...
| 4.19 | **Las fotos y las notas de voz del informe de mensajes** | El CSV `mensajes` (E18, 11/9) trae cuándo, quién, de qué combi, al grupo o en privado, el texto o el pie de foto, la duración y si el medio SIGUE guardado — y deja fuera el contenido de la voz y la foto **a propósito**: son data-URL en base64 de hasta 2 MB y una sola fila de esas hace un archivo que no abre en ninguna planilla. El día que alguien pida las fotos, la respuesta es un ZIP con los archivos o un enlace por mensaje, **no** meter base64 en el CSV. Hay que decidirlo antes de que lo pidan, no improvisarlo ese día (repaso del 21/9) |
...
- [ ] **Conectar el dominio** — `microstempo.com` ya está comprado (3/9),
      ...
      el orden que no rompe nada están en `DOMINIO.md`. La landing y el
      paquete `com.microstempo.chofer` ya están en el repo.
...
cerrar el socket con el HTTP vivo ya no marca «sin señal». **Tanda 3 (8/9,
paneles, sin APK): hecha** — el service worker no guarda nada que viaje con
token ni la marca, los números, el perfil, las grabaciones, los informes o
el creador (`CACHE_NAME` v54); Babel con `integrity` en Despacho y el
creador; el gerente no pierde la sesión al recargar; al reconectar se vuelve
a la ruta que se miraba; «Salir» revoca el token en el servidor (Despacho y
...
primera —**el turno de 8 horas** con el GPS y la pantalla bloqueada— quedó
contestada el 7/9 (ver 1.3): 13 horas seguidas sin un corte; falta la
batería. La otra —**las tiles del mapa**— quedó resuelta en tres capas:
