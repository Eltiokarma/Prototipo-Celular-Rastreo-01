...
| --- | --- | --- | --- |
| 1.1 | **Cambiar `CREATOR_PASSWORD`** | Estuvo escrita en un chat en texto plano y es la ÚNICA llave que abre TODAS las cooperativas. Todo lo demás de esta lista da lo mismo si esto queda. Decisión del dueño (8/8): se cambia **antes de exponerlo**, no durante el desarrollo | 10 minutos |
...
| 4.6 | **El acumulado por unidad no tiene corte por fecha** | Re-medido a escala real (9/8, `COSTOS.md` §3). `/admin/metrics` es la única lectura del sistema que agrupa "todo lo retenido" en vez de un período: **1003 ms a 5000 unidades**, y con la ingesta de GPS a la par eso **congela a la flota entera 1211 ms**. Crece lineal (1,5×), así que no es urgente por forma, sí por tamaño. Los índices ya no compran más — la salida es acotarlo a un rango, como el resto del panel. **NO se hizo porque cambia lo que se ve**: el "acumulado" dejaría de ser de todo lo retenido y pasaría a ser de un período. Es decisión del dueño | acotar: 1 h |
| 4.8 | **Números a 90 días congela la flota 2,5 s** | Medido (9/8). `/gerencia/resumen` a 90 días tarda 1746 ms y **detiene la ingesta de GPS de toda la flota 2493 ms** — el peor número del sistema. Crece lineal, y la pestaña **abre con 7 días por defecto** (61 ms), así que es una elección deliberada y ocasional, no el camino diario. El arreglo NO cambia nada de lo que se ve: hoy trae 244 980 filas a JavaScript y las agrupa en tres pasadas; **agregando en SQL** la misma cuenta baja de 654 ms (512 traer + 142 agrupar) a **182 ms**. Es una reescritura grande de una pantalla con lógica de cumplimiento delicada, así que se deja medida y no hecha: el riesgo de cambiarle los números al gerente sin querer es peor que 2,5 s ocasionales | reescribir: medio día |
