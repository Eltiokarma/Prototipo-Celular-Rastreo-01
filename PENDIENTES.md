...
| 4.18 | ~~**Los trucos del chofer contra el control: un análisis a fondo**~~ | **ANÁLISIS HECHO** (10/9): `TRUCOS-2026-09-10.md`, doce trucos, cada uno con cómo se ve en los datos, qué hace el sistema hoy (con archivo), si se detecta, qué hacer y qué NO se va a hacer. Principio: detectar no es acusar; todo se anota, poco se avisa, nada se castiga solo. **Primer paso hecho**: el GPS simulado (T2) — Android marca la posición que sale de una app de «fake GPS», la app la manda como `simulado`, la unidad lleva `gpsSimulado`, Despacho lo ve y la auditoría lo anota (APK, `versionCode` 4). **Falta, en este orden**: (1) servidor, sin APK: la tabla `huecos` (cortes de señal por unidad, con dónde se cortó y dónde reapareció) y la tabla de presencia con minutos ausente por día y «ausente en movimiento sobre el trazado», más los saltos imposibles y `relojAtrasadoS`/`descartadas` en el informe por unidad — una sección «Señal y presencia» en el informe de Despacho; (2) APK: mandar `accuracy` y no juzgar desvío ni parada con posiciones de más de 100 m; (3) después: la heurística de simulación sin flag y el contador de avisos de tráfico | ✔ análisis |
