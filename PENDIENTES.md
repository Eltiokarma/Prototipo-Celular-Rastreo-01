...
| 4.7 | **`shifts` no se poda nunca** | Encontrado midiendo (8/8). Todas las tablas de historial tienen retención —vueltas y tramos 120 días, chat 30, SOS 365, auditoría 1000 por empresa— menos los turnos, que crecen para siempre: a 2000 unidades son ~320 000 filas a los 120 días y siguen. Hoy no duele (`/admin/shifts` sale en 20 ms porque filtra por fecha), y por eso no es urgente; pero es la única tabla sin techo y el día que moleste va a ser en el disco, no en la pantalla. Sumar `SHIFTS_DIAS` a `podarHistorico` |
